...
// Google Maps Static API — ganti dengan API key kamu
const MAPS_KEY = 'YOUR_GOOGLE_MAPS_STATIC_API_KEY';

function staticMapUrl(lat: number, lng: number, zoom = 15, w = 600, h = 300) {
  // Jika belum punya API key, fallback ke OpenStreetMap tile (gratis, tanpa key)
  if (MAPS_KEY === 'YOUR_GOOGLE_MAPS_STATIC_API_KEY') {
    // OpenStreetMap Static via osm-static-maps (open source, no key)
...
export default function AddressBottomSheet({ visible, onClose, onSave }: Props) {
  ...
  // Geser pin manual (simulasi: geser ±0.001 deg per tap)
  ...
  const mapUri = staticMapUrl(pin.lat, pin.lng);
              ...
              <React.Fragment key={i}>
                <View style={styles.stepItem}>
                  <View style={[styles.stepCircle, {
                    backgroundColor: (active || done) ? t.primary : t.border,
                  }]}>
                    {done
...
function Field({ label, value, onChangeText, placeholder, error, t, keyboardType, multiline }: {
  ...
  return (
    <View style={{ marginBottom: 14 }}>
      <Text style={[styles.fieldLabel, { color: t.text }]}>{label} <Text style={{ color: '#EF4444' }}>*</Text></Text>
      <TextInput
        style={[styles.fieldInput, {
          backgroundColor: t.surface, borderColor: error ? '#EF4444' : t.border, color: t.text,
          height: multiline ? 80 : 48, textAlignVertical: multiline ? 'top' : 'center',
        }]}
        placeholder={placeholder} placeholderTextColor={t.textSub}
...
const styles = StyleSheet.create({
  ...
  // steps
  stepBar:           { flexDirection:'row', alignItems:'center', justifyContent:'center', paddingVertical:12, gap:0, borderBottomWidth:1 },
  ...
  pinOverlay:        { position:'absolute', top:0, left:0, right:0, bottom:0, alignItems:'center', justifyContent:'center', pointerEvents:'none' },
