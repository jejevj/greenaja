...
export type Voucher = {
  ...
  type: VoucherType; value: number; minPurchase: number;
  maxDiscount?: number; expiry: string; status: VoucherStatus; usedAt?: string;
};
...
interface AppState {
  ...
  updateNotif:    (n: Partial<NotifSettings>) => void;
  // computed
...
const DUMMY_ORDERS: Order[] = [
  { id: 'GRN-20260622-8471', date: '22 Jun 2026', status: 'packing',
    items: [{ name: 'Bayam Segar', qty: 2 }, { name: 'Tomat Organik', qty: 1 }], total: 42000 },
  { id: 'GRN-20260618-3102', date: '18 Jun 2026', status: 'delivered',
    reviewed: true, reviewRating: 5,
    items: [{ name: 'Kangkung Segar', qty: 3 }], total: 27500 },
  { id: 'GRN-20260610-5577', date: '10 Jun 2026', status: 'delivered',
    reviewed: false, items: [{ name: 'Brokoli Organik', qty: 1 }], total: 35000 },
];
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const updateNotif   = (n: Partial<NotifSettings>) =>
    setNotifState(prev => ({ ...prev, ...n }));
  ...
  return (
    <AppContext.Provider value={{
      ...
      activeVoucherCount, profileBadgeCount, profileBadgeAlert,
    }}>
      {children}
    </AppContext.Provider>
  );
