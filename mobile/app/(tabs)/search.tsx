...
export default function SearchScreen() {
  ...
  const handleSubmit = () => { Keyboard.dismiss(); setFocused(false); };
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: t.bg }]} edges={['top']}>
      ...
      {showResults && (
        <>
          {/* Hanya teks jumlah — TANPA tombol Lihat semua */}
          <View style={styles.resultHeader}>
            <Text style={[styles.resultCount, { color: t.textSub }]}>
              <Text style={{ color: t.primary, fontWeight: '800' }}>{results.length}</Text>
              {' '}produk ditemukan untuk{' '}
              <Text style={{ color: t.text, fontWeight: '700' }}>"{query}"</Text>
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  // hasil
  resultHeader: { paddingHorizontal: 20, paddingBottom: 12, paddingTop: 4 },
  resultCount:  { fontSize: 13, lineHeight: 20 },
