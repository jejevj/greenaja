...
import {
  ...
  useColorScheme, Dimensions, FlatList,
} from 'react-native';
...
export const ALL_PRODUCTS: (ProductSheetItem & { category: string; rating: number; sold: number })[] = [
  { id:'1',  name:'Bayam Segar',       seller:'GreenAja', tag:'Bestseller', category:'Sayuran', rating:4.9, sold:312, description:'Bayam lokal segar dipanen pagi hari langsung dari kebun organik Bogor.',        variants:[{id:'v1',label:'1 Ikat',        price:4500,  unit:'ikat', stock:30},{id:'v2',label:'3 Ikat',        price:12000,unit:'ikat', stock:15},{id:'v3',label:'5 Ikat',        price:18000,unit:'ikat',stock:8}]  },
  { id:'2',  name:'Tomat Organik',     seller:'GreenAja', tag:'Organik',    category:'Sayuran', rating:4.8, sold:204, description:'Tomat cherry organik bersertifikat, tanpa pestisida, rasa manis alami.',       variants:[{id:'v1',label:'250g',          price:7500,  unit:'gram',stock:20},{id:'v2',label:'500g',          price:12000,unit:'gram',stock:12},{id:'v3',label:'1kg',           price:22000,unit:'kg',  stock:0}]   },
  { id:'3',  name:'Cabai Rawit',       seller:'GreenAja', tag:'Populer',    category:'Rempah',  rating:4.7, sold:178, description:'Cabai rawit merah segar, tingkat kepedasan tinggi, cocok untuk masakan rumah.',  variants:[{id:'v1',label:'100g',          price:6000,  unit:'gram',stock:25},{id:'v2',label:'250g',          price:14000,unit:'gram',stock:10},{id:'v3',label:'500g',          price:26000,unit:'gram',stock:5}]   },
  { id:'4',  name:'Kangkung',          seller:'GreenAja', tag:'Segar',      category:'Sayuran', rating:4.6, sold:156, description:'Kangkung air segar, batang renyah, cocok untuk tumis dan lalapan.',           variants:[{id:'v1',label:'1 Ikat',        price:3500,  unit:'ikat', stock:40},{id:'v2',label:'3 Ikat',        price:9500, unit:'ikat', stock:20}]  },
  { id:'5',  name:'Wortel Baby',       seller:'GreenAja', tag:'Baru',       category:'Sayuran', rating:4.8, sold:98,  description:'Wortel baby Lembang, manis dan renyah, cocok untuk camilan sehat anak.',      variants:[{id:'v1',label:'250g',          price:9000,  unit:'gram',stock:15},{id:'v2',label:'500g',          price:16500,unit:'gram',stock:8}]   },
  { id:'6',  name:'Brokoli Hijau',     seller:'GreenAja', tag:'Organik',    category:'Sayuran', rating:4.9, sold:134, description:'Brokoli segar organik ukuran besar, kaya vitamin C dan serat.',               variants:[{id:'v1',label:'Kecil (~300g)', price:8000,  unit:'buah',stock:12},{id:'v2',label:'Besar (~600g)', price:15000,unit:'buah',stock:6}]   },
  { id:'7',  name:'Jahe Merah',        seller:'GreenAja', tag:'Organik',    category:'Rempah',  rating:4.7, sold:87,  description:'Jahe merah organik kering, kaya manfaat, cocok untuk minuman kesehatan.',     variants:[{id:'v1',label:'100g',          price:8500,  unit:'gram',stock:30},{id:'v2',label:'250g',          price:19000,unit:'gram',stock:15}]  },
  { id:'8',  name:'Pisang Cavendish',  seller:'GreenAja', tag:'Segar',      category:'Buah',    rating:4.6, sold:211, description:'Pisang Cavendish manis, matang sempurna, dipanen langsung dari kebun.',       variants:[{id:'v1',label:'1 Sisir',       price:12000, unit:'sisir',stock:20},{id:'v2',label:'1 Tandan',       price:35000,unit:'tandan',stock:8}]  },
  { id:'9',  name:'Mangga Harum Manis',seller:'GreenAja', tag:'Populer',    category:'Buah',    rating:4.9, sold:265, description:'Mangga harum manis kualitas premium, manis legit, tanpa serat berlebih.',     variants:[{id:'v1',label:'500g',          price:14000, unit:'gram',stock:18},{id:'v2',label:'1kg',           price:25000,unit:'kg',  stock:9}]   },
  { id:'10', name:'Kunyit Segar',      seller:'GreenAja', tag:'Lokal',      category:'Rempah',  rating:4.5, sold:63,  description:'Kunyit segar lokal, warna orange cerah, untuk masakan dan jamu tradisional.', variants:[{id:'v1',label:'100g',          price:5000,  unit:'gram',stock:35},{id:'v2',label:'250g',          price:11000,unit:'gram',stock:20}]  },
  { id:'11', name:'Bayam Merah',       seller:'GreenAja', tag:'Organik',    category:'Sayuran', rating:4.7, sold:72,  description:'Bayam merah organik kaya antioksidan, cocok untuk salad dan smoothie.',       variants:[{id:'v1',label:'1 Ikat',        price:6000,  unit:'ikat', stock:22}]  },
  { id:'12', name:'Jeruk Nipis',       seller:'GreenAja', tag:'Segar',      category:'Buah',    rating:4.6, sold:189, description:'Jeruk nipis segar berair, asam segar, cocok untuk minuman dan masakan.',      variants:[{id:'v1',label:'250g (~5 bj)', price:7000,  unit:'gram',stock:40},{id:'v2',label:'500g (~10 bj)',price:12500,unit:'gram',stock:25}]  },
];
...
export default function ProductsScreen() {
  ...
  const { category: initCat = 'Semua', query: initQuery = '' } =
    useLocalSearchParams<{ category?: string; query?: string }>();

  const [activeCategory, setActiveCategory] = useState(initCat as string);
  ...
  const filtered = useMemo(() => {
    ...
    if (initQuery) {
      const q = (initQuery as string).toLowerCase();
      list = list.filter(p =>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: t.bg }]} edges={['top']}>
        ...
        <View style={styles.chipWrap}>
          {SORTS.map(s => {
            ...
            return (
              ...
              >
                {s.key === 'price_asc'  && <Ionicons name="arrow-up-outline"   size={12} color={active ? t.primary : t.textSub} />}
                {s.key === 'price_desc' && <Ionicons name="arrow-down-outline" size={12} color={active ? t.primary : t.textSub} />}
                {s.key === 'rating'     && <Ionicons name="star-outline"        size={12} color={active ? t.primary : t.textSub} />}
                {s.key === 'popular'    && <Ionicons name="flame-outline"       size={12} color={active ? t.primary : t.textSub} />}
      ...
      {filtered.length === 0 ? (
        <View style={styles.emptyWrap}>
          ...
          <Text style={[styles.emptySub,   { color: t.textSub }]}>Coba kata kunci atau filter lain</Text>
        </View>
      ) : (
        <FlatList
          data={filtered}
              ...
              >
                ...
                <View style={styles.cardBody}>
                  ...
                  <View style={styles.sellerRow}>
                    <Ionicons name="storefront-outline" size={11} color={t.primary} />
                    <Text style={[styles.cardSeller, { color: t.primary }]} numberOfLines={1}>{p.seller}</Text>
                  </View>
                  <View style={styles.ratingRow}>
                    <Ionicons name="star" size={11} color="#F59E0B" />
                    <Text style={[styles.ratingText, { color: t.textSub }]}>
...
const styles = StyleSheet.create({
  ...
  sellerRow:     { flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 },
  cardSeller:    { fontSize: 11, fontWeight: '600' },
