...
function formatMinPurchase(v: Voucher) {
  if (v.minPurchase === 0) return 'Tanpa minimum belanja';
...
function VoucherDetailModal({
  ...
  const terms = getVoucherTerms(voucher);
            ...
            <View style={styles.modalGradTop}>
              ...
              {!isActive && (
                <View style={[styles.modalTypeChip, { backgroundColor: 'rgba(0,0,0,0.25)' }]}>
                  <Text style={styles.modalTypeText}>{voucher.status === 'used' ? 'TERPAKAI' : 'KADALUARSA'}</Text>
                </View>
              )}
            </View>
    ...
    >
      ...
      <View style={styles.cardContent}>
        <View style={styles.cardTop}>
          <Text style={[styles.cardValue, { color: isActive ? t.primary : t.textSub }]}>{formatValue(voucher)}</Text>
          {voucher.status === 'used' && (
        ...
        <View style={styles.cardFooter}>
          ...
          <View style={[styles.codeChip, { backgroundColor: isActive ? t.primaryMuted : t.accent }]}>
            <Text style={[styles.codeChipText, { color: isActive ? t.primary : t.textSub }]}>{voucher.code}</Text>
          </View>
        </View>
      </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: t.bg }]} edges={['top']}>
      <View style={[styles.header, { borderBottomColor: t.border }]}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.pageSub, { color: t.textSub }]}>{activeVoucherCount} voucher aktif tersedia</Text>
        </View>
